...
def test_train_returns_float(tmp_path):
    ...
    assert 0.0 <= f1 <= 1.0, f"F1 score {f1} out of range [0, 1]"


def test_report_file_created():
    """Kiem tra file outputs/report.json duoc tao sau khi huan luyen."""
    # Tao data truc tiep trong thu muc tam cua pytest
    import tempfile
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        eval_path = os.path.join(tmp_dir, "holdout.csv")
        df.iloc[:160].to_csv(train_path, index=False)
        df.iloc[160:].to_csv(eval_path, index=False)

        # Chay train trong thu muc tam
        ...
        report_path = os.path.join(tmp_dir, "outputs", "report.json")
        assert os.path.exists(report_path), f"Report file not found at {report_path}"
        with open(report_path) as f:
            report = json.load(f)
        assert "f1_score" in report, "f1_score not in report"
        assert "accuracy" in report, "accuracy not in report"
...
def test_model_file_created():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        model_path = os.path.join(tmp_dir, "models", "model.joblib")
